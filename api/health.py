...
def _check_llm_primary() -> DependencyStatus:
    ...
    try:
        ...
        return DependencyStatus(name="llm_primary", status="unhealthy", detail=str(e)[:200])
...
def _check_file_storage() -> DependencyStatus:
    ...
    try:
        ...
        if missing:
            return DependencyStatus(name="file_storage", status="degraded", detail=f"missing dirs: {missing}")
...
async def detailed_health():
    ...
    return DetailedHealthResponse(status=overall, version="2.0", uptime_seconds=round(uptime, 1), dependencies=deps)
