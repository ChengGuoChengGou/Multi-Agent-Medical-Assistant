...
class SpeechRequest(BaseModel):
    ...
    voice_id: str = Field(
        default="EXAMPLE_VOICE_ID", description="ElevenLabs voice ID to use", examples=["21m00Tcm4TlvDq8ikWAM"]
    )
            ...
            "examples": [
                {
                    ...
                    "dedup_stats": {"active_requests": 2, "total_deduped": 15, "cache_size": 8},
                    "api_auth": {"dev_mode": True, "keys_configured": 0, "auth_successes": 0, "auth_failures": 0},
                }
            ]
        }
    )
...
class TranscribeResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"examples": [{"transcript": "Patient presents with persistent cough and fever for 3 days."}]}
    )
