...
def _check_required_env(result: ValidationResult):
    ...
    api_key = os.getenv("openai_api_key") or os.getenv("OPENAI_API_KEY") or os.getenv("embedding_openai_api_key")
    if not api_key:
        result.add_error("No LLM API key found. Set OPENAI_API_KEY or openai_api_key in .env")
...
def _check_model_config(result: ValidationResult):
    ...
    if not model_name:
        result.add_warning("model_name not set, defaulting to gpt-4o-mini")
...
def _check_mcp_config(result: ValidationResult):
    ...
    for name, (cmd, _) in mcp_commands.items():
        if not shutil.which(cmd):
            result.add_warning(f"MCP server '{name}' requires '{cmd}' not found in PATH")
