...
COMPLEX_QUERY_INDICATORS = [
    ...
    "chest pain", "shortness of breath",  # + neurological / GI etc.
    "fever", "rash",
    "headache", "vision",
...
def _analyze_query_complexity(query: str) -> Dict[str, Any]:
    ...
    features = {
        ...
        "has_symptoms": any(w in q for w in ["pain", "ache", "fever", "cough", "fatigue", "nausea", "rash", "swelling"]),
...
def exploration_stage(query: str, has_image: bool = False, image_type: str = None) -> Dict[str, Any]:
    ...
    if features["has_drug_ref"]:
        findings["suggested_agents"].append({
            "agent": "MCP_AGENT",
            "reason": "Drug information / interaction lookup",
            "priority": "high"
        })
    ...
    if features["has_symptoms"] or features["has_chronic"]:
        findings["suggested_agents"].append({
            "agent": "RAG_AGENT",
            "reason": "Medical knowledge retrieval for symptom analysis",
            "priority": "high"
        })
        findings["suggested_agents"].append({
...
def planning_stage(
    ...
    for agent_info in suggested:
        ...
        priority = StepPriority(priority_str) if priority_str in [p.value for p in StepPriority] else StepPriority.MEDIUM
...
def verification_stage(
    ...
    # ── Safety checks ──
    safety_disclaimers = [
        "consult", "healthcare", "doctor", "professional", "emergency",
        "seek medical", "not a substitute", "educational"
    ]
    ...
    dangerous_patterns = [
        "stop taking", "discontinue", "ignore your doctor",
        "don't need", "no need to see", "home remedy is enough"
    ]
...
def create_diagnostic_plan(
    ...
    logger.debug(f"[PLANNER] Exploration complete. Complexity: {findings['complexity']}, "
...
def check_refresh_needed() -> Dict[str, bool]:
    ...
    now = time.time()
    checks = {
        "rag_reindex_needed": (now - _refresh_config.last_rag_refresh) > (_refresh_config.rag_reindex_interval_hours * 3600),
        "seed_check_needed": (now - _refresh_config.last_seed_check) > (_refresh_config.vector_seed_check_interval_hours * 3600),
